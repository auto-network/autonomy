...
def _apply_workspace_mount_settings(
    ...
    """Extend *mounts* with ``autonomy.workspace.mount#2`` org-scoped mounts.

    Each mount's ``subpath`` is resolved under ``orgs/<workspace-org>/`` in the
    autonomy-orgs volume, realpath-refused if it escapes that tree, type-checked
    against ``kind``, and translated to a host path for the bind (see
    :func:`_resolve_org_mount`). Required mounts whose target is absent raise
    :class:`WorkspaceMountMissingError`; optional absent ones are skipped. A
    present-but-wrong-type or escaping target raises
    :class:`WorkspaceMountInvalidError`.
    """
    if not workspace.mounts:
        return
    org = getattr(workspace, "graph_project", None)
    if not org:
        # No org to scope under — every mount would be unresolvable. Surface the
        # first as the representative failure rather than silently dropping all.
        first = next(iter(workspace.mounts))
        raise WorkspaceMountInvalidError(
            mount_key=first,
            reason="workspace has no org (graph_project) to scope its mounts under",
        )
