...
You are running inside the `autonomy-agent:enterprise` container. The
...
git, and runs `poetry install` in `/workspace/enterprise`.

## Worktrees

- `/workspace/enterprise` — Anchore Enterprise (writable)
...
exit status to `/workspace/output/.setup-exit` when done. Before running
...
If `.setup-exit` is missing, setup is still running. Check `.setup.log` for
progress. If it is non-zero, read the log and halt — don't try to work
around a broken environment.

## Docker-in-Docker

A full Docker daemon runs inside this container (storage driver `vfs`,
`--tls=false`). Use `docker`, `docker compose`, etc. normally. The outer
host's docker is not visible.

## Graph scoping

`GRAPH_SCOPE=anchore` is set so notes and searches default to the Anchore
project boundary. `GRAPH_TAGS=enterprise` is pre-applied to notes you
create.
