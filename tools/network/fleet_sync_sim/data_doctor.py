...
1. Foreign-key orphans in NOT NULL FK tables -- a row whose required
   ...
   receiver and get skipped+quarantined during install (see alpha.py);
...
Usage:
    python3 -m tools.network.fleet_sync_sim.data_doctor <db_path> [--json]
    python3 -m tools.network.fleet_sync_sim.data_doctor <db_path> --delete-orphans [--yes]
"""
...
#: row is unrepresentable and safe to delete -- the same class alpha.py
