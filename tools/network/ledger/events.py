...
def _v_delegate(p: dict) -> None:
    ...
    _require_str(p["grant_nonce"], "delegate.grant_nonce",
                 max_len=GRANT_NONCE_HEX_LEN)
    try:
        _decode_hex(p["grant_nonce"], GRANT_NONCE_HEX_LEN, "delegate.grant_nonce")
    except _IdkitMalformed as exc:
        raise SchemaError(str(exc)) from None
...
DELEGATE_CONSENT_DOMAIN = b"autonomy.ledger.delegate-consent.v2\n"

#: A grant nonce is 32 random bytes, hex-encoded. Single-use per child key:
#: the fold admits at most one delegate event per (child_pub, grant_nonce).
GRANT_NONCE_HEX_LEN = 64


def mint_grant_nonce() -> str:
    """A fresh single-use grant nonce. Child-generated by preference (the
    signer of the consent proof), but either side may mint it — the child
    signs the whole proof input and the fold enforces uniqueness, so who
    chose the nonce carries no security weight."""
    import os

    return os.urandom(GRANT_NONCE_HEX_LEN // 2).hex()
...
def delegate_proof_input(
    ...
    *,
    ...
    """The bytes a delegate's consent proof covers (auto-le0kg, v2).

    Binds the organization's GENESIS, the ISSUING member's key, the child
    key itself and the FULL GRANT TERMS — scopes, ``can_redelegate``,
    ``ttl`` — plus a SINGLE-USE ``grant_nonce``. The terms are covered so
    a grant the child consented to cannot be reissued with wider
    authority under the child's own signature; the nonce is what makes
    consent REVOCABLE rather than perpetual — without it, a revoked grant
    can be republished byte-identically with the original proof, and the
    republication carries the revocation in its ancestry so the kill
    check never reaches it.

    The scope SORT IS PART OF THE BINDING, not tidiness: a scope set has
    no order, and signing the caller's list order would make the same
    grant produce two different signatures. ``ttl`` binds the grant's
    DURATION and is omitted from the input when absent, mirroring the
    payload; the expiry position needs no binding because nonce
    uniqueness already refuses a re-publication at any later position.
    """
    body = {
        "genesis_id": genesis_id,
    ...
    if ttl is not None:
        body["ttl"] = int(ttl)
    return DELEGATE_CONSENT_DOMAIN + canonical_json(body)
...
def sign_delegate_proof(
    ...
    *,
    ...
    possession AND consent over the grant's full terms. Possession is what
    makes concurrent grants over one key equally legitimate rather than
    one of them forged, and stops a member publishing a grant over a
    public key they merely observed; the terms-plus-nonce binding is what
    makes that consent specific to THIS grant and withdrawable (see
    :func:`delegate_proof_input`). Every renewal therefore requires a
    fresh signature from the child — an issuer can no longer extend a
    delegate's lifetime alone."""
    return child_key.sign_hex(
        delegate_proof_input(
            genesis_id, issuer_key, child_key.public_hex, scope,
            can_redelegate=can_redelegate, ttl=ttl, grant_nonce=grant_nonce,
        )
    )
