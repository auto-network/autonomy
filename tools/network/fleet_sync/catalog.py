...
class MutationCatalog:
    ...
    def items_at_addresses(self, address_blobs: Sequence[bytes]) -> list[AuthoredMutation]:
        """The current row at each catalog address, rebuilt with the origin
        and transaction that authored it, for a targeted re-fetch
        (row_fetch.py). An address this store does not hold, or cannot
        resolve, is skipped: the asker learns that from the count."""
        items: list[AuthoredMutation] = []
        for blob in address_blobs:
            raw = self.conn.execute(
                "SELECT c.timestamp_ns, c.tombstone, c.operation_index, "
                "o.incarnation, t.transaction_id FROM fleet_sync_catalog c "
                "JOIN fleet_sync_transactions t ON t.id=c.transaction_ref "
            ...
            if raw is None:
                continue
            try:
                table, address = self._decode_address(bytes(blob))
            except WatermarkError:
                continue
            incarnation, transaction_id = str(raw[3]), str(raw[4])
            if not self._serveable(table, address, transaction_id):
                continue
            if bool(raw[1]):
                mutation = Mutation(table, address, int(raw[0]), True)
            else:
                try:
                    row = self._live_row(self.conn, table, address)
                except WatermarkError:
                    continue
                mutation = Mutation(
                    table, address, int(raw[0]), False,
                    _logical_values(TABLE_POLICIES[table], row),
                )
            items.append(AuthoredMutation(incarnation, transaction_id, int(raw[2]), mutation))
        return items

    def clear_quarantined_address(self, table: str, address: tuple) -> bool:
        """Drop the quarantine entry at one address after its row landed by
        another path, and walk the origin cursors the hold was pinning."""
        ensure_quarantine_table(self.conn)
        blob = encode_value([table, list(address)])
        row = self.conn.execute(
            "SELECT origin FROM fleet_sync_quarantine WHERE address=?", (blob,)
        ).fetchone()
        if row is None:
            return False
        with self.conn:
            self.conn.execute("DELETE FROM fleet_sync_quarantine WHERE address=?", (blob,))
            origin_id = self._origin_id(str(row[0])) if row[0] else None
            if origin_id is not None:
                ensure_origin_cursor_schema(self.conn)
                self._advance_cursor(origin_id)
        return True
    ...
    def frameless_hold_addresses(self) -> list[bytes]:
        """Quarantined rows that hold an origin's cursor and stored no frame
        (parked by a bootstrap sweep): the ones only a re-fetch can land."""
        ensure_quarantine_table(self.conn)
        holders = ",".join("?" * len(CURSOR_HOLDING_REASONS))
        return [
            bytes(r[0]) for r in self.conn.execute(
                f"SELECT address FROM fleet_sync_quarantine WHERE reason IN ({holders}) "
                "AND frame IS NULL ORDER BY quarantined_at_ns",
