...
1. One write on one machine, then nothing. The floor tables of all six org
   stores are sampled every round for 60 rounds. Each store must see each
   origin's floor move exactly once (the writer seals after writing, every
   other machine seals once after applying the row, and every store learns
   each new floor once), each persona floor must move at most twice (once per
   machine of the persona whose floor it folds), and after the last change
   the tables must not move again for the rest of the window. The persona
   floor in every store must end at or above the written row's timestamp.
2. Negative control: no write at all. 60 rounds with no floor change anywhere.
...
def test_one_write_then_the_fleet_is_silent(tmp_path: Path) -> None:
    org = _bring_up(tmp_path)
    try:
...
def test_no_write_means_no_floor_and_no_frame(tmp_path: Path) -> None:
    org = _bring_up(tmp_path)
    try:
        settled, quiet_since = _settle(org)
        baseline = _baseline_bytes(_org_pulls(org, quiet_since))
        start_wall = time.time()
        changes, _stamps = _observe(org, settled, WINDOW_ROUNDS)
        (tmp_path / "silence-idle.json").write_text(json.dumps(
            {"baseline_bytes": baseline, "changes": changes}, indent=1, default=str))
        assert changes == [], ("floors moved with no write", changes[:8])
        pulls = _org_pulls(org, start_wall)
        assert pulls, "the workers stopped pulling"
