...
from tools.network.fleet_sync.codec import decode_stream
from tools.network.fleet_sync.snapshot import encode_snapshot
...
def _seed(db: GraphDB, *, reverse: bool, local_prefix: str) -> None:
    ...
    # Personal identity is ordinary fleet-wide personal data.
    db.conn.execute(
        "INSERT INTO settings(id,set_id,schema_revision,key,payload,publication_state,"
        "created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",
        (
            "root", "autonomy.identity.personal", 1, "root",
            '{"armor":"ciphertext"}', "raw",
...
def test_equivalent_real_schema_databases_emit_identical_bytes(tmp_path: Path) -> None:
    ...
    assert b"autonomy.identity.personal" in left_bytes
