...
def test_refusing_a_checkpoint_does_not_disable_the_sweep() -> None:
    """accept_checkpoint=False means "no CHECKPOINT", not "no bootstrap".

    A resuming sweep sets that flag precisely to refuse a checkpoint. If the
    serve evaluated the refusal before the sweep branch it would clear
    wants_checkpoint and the sweep would never be served -- the pin protecting
    the sweep would disable it, and a resuming bootstrap could never finish.

    STRUCTURAL: it reads the compiled serve generator and requires the sweep
    decision to be bound BEFORE the refusal consumes it. It does not prove a
    live server served a sweep to a refusing peer.
    """
    import dis
    import types
    ...
    target = None
    for code in _walk(FleetSyncScheduler._handle.__code__):
        # accept_checkpoint is a closure FREEVAR in the serve generator,
        # not a local, so co_names|co_varnames alone does not see it.
        names = (
            set(code.co_names) | set(code.co_varnames)
            | set(code.co_cellvars) | set(code.co_freevars)
        )
        if "serve_sweep" in names and "accept_checkpoint" in names:
            target = code
            break
    assert target is not None, (
        "the serve path no longer binds a sweep decision alongside the "
        "checkpoint refusal"
    )

    order = [
        i.argval for i in dis.get_instructions(target)
        if i.argval in {"serve_sweep", "accept_checkpoint"}
    ]
    assert "serve_sweep" in order
    first_sweep = order.index("serve_sweep")
    first_refusal = order.index("accept_checkpoint")
    assert first_sweep < first_refusal, (
        "the checkpoint refusal is evaluated before the sweep decision, so a "
        "peer refusing checkpoints would never be served a sweep"
    )
