...
def build_link_server_hello(
    link_key: KeyPair, *, org: str, token: str, client_eph: str,
) -> Tuple[X25519PrivateKey, bytes, bytes]:
    """Build the only public-link hello: proof of the fragment key."""
    private_key = X25519PrivateKey.generate()
...
def build_certificate_server_hello(
    signing_key: KeyPair, cert: DelegationCert, *, org: str, token: str,
    client_eph: str,
) -> Tuple[X25519PrivateKey, bytes, bytes]:
    """Build a certificate hello for explicitly typed non-link channels."""
    private_key = X25519PrivateKey.generate()
    server_eph = _eph_pub_hex(private_key)
...
def verify_link_server_hello(raw, *, link_pub: str, org: str, token: str,
                             client_eph: str) -> Tuple[str, bytes]:
    """Verify the exact public-link hello against its fragment key."""
    data = _parse_hello(raw, frozenset({"v", "eph_pub", "link_sig"}),
                        "SERVER_HELLO")
    _decode_eph_pub(data.get("eph_pub"), "server eph_pub")
...
def verify_certificate_server_hello(raw, *, root_pub: str, org: str,
                                    token: str, client_eph: str,
                                    now: Optional[int] = None) -> Tuple[str, bytes]:
    """Verify the exact certificate hello for a typed non-link channel."""
