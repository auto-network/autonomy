...
// Generated by `graph set typegen --plugin {plugin_id}`.
// To regenerate, run that command. The pre-commit drift check
// (`graph set typegen --plugin {plugin_id} --check`) fails CI if
...
def render_dts(plugin_id: str, sources: Iterable[SchemaSource]) -> str:
    """Render the full ``.d.ts`` body for a plugin's schemas.
    ...
    header = HEADER_TEMPLATE.format(
        plugin_id=plugin_id, source_lines=source_lines,
    )
...
def cmd_set_typegen(args) -> None:
    ...
    if not args.plugin:
        sys.stderr.write(
            "typegen: --plugin <id> required (or --all to iterate every plugin)\n"
        )
        sys.exit(2)
