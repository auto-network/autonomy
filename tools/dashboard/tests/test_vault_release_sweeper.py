...
def store(tmp_path, monkeypatch):
    """Machine-store isolation: release leases are machine-homed Settings
    (autonomy.vault.release-lease), so the tests isolate by rooting the
    data tree in tmp rather than by passing a database path around."""
...
def _deliver(root: Path, *, id, session, expires_at, now, body=b"s3cr3t"):
    ...
    vault_releases.record_release(
        ...
        container_path=f"/run/secrets/{id}.secret", host_path=str(host_path), now=now,
    )
    ...
    for mode in ("mediated", "client_operation", "open"):
        with pytest.raises(ValueError, match="delivered"):
            vault_releases.record_release(
                ...
                expires_at=1, container_path="/c", host_path="/h",
            )
...
def test_a_duplicate_release_id_is_refused(store):
    vault_releases.record_release(
        ...
        expires_at=1, container_path="/c", host_path="/h", now=0,
    )
...
def test_shred_is_idempotent_and_keeps_the_first_reason(store):
    ...
    assert vault_releases.mark_shredded("r", reason="expired", now=10)
    # A re-sweep must not rewrite when or why the secret died.
    assert not vault_releases.mark_shredded("r", reason="orphaned",
                                            now=99)
    rec = vault_releases.get("r")
...
def test_a_crash_at_any_owned_point_leaves_no_undestroyed_secret(
    ...
    if point == "before_record":
        ...
        assert vault_releases.get(sid) is None
    ...
    if point == "after_record_before_file":
        ...
        vault_releases.record_release(
            ...
            container_path="/c", host_path=str(host_path), now=0,
        )
        assert not host_path.exists()
        sweeper.sweep(session_exists=live, delivery_root=delivery_root, now=200)  # past deadline
        assert vault_releases.get(sid)["shred_reason"] == "expired"
        return

    # after_file / mid_unlink both start from a fully delivered release.
    _deliver(delivery_root, id=sid, session=session, expires_at=100,
             now=0)
    ...
    if point == "mid_unlink":
        ...
        host_path.unlink()
    sweeper.sweep(session_exists=live, delivery_root=delivery_root, now=200)
    assert not host_path.exists()
    assert vault_releases.get(sid)["shredded_at"] is not None


# ── Deadline, session end, orphan, container-gone ───────────────────────

def test_a_release_is_destroyed_at_its_deadline(store, delivery_root):
    host = _deliver(delivery_root, id="r", session="s",
                    expires_at=1000, now=0)
    live = lambda s: True
    # Before the deadline: left in place, the session is still entitled.
    sweeper.sweep(session_exists=live, delivery_root=delivery_root, now=500)
    assert host.exists()
    assert vault_releases.get("r")["shredded_at"] is None
    # At/after the deadline: destroyed.
    sweeper.sweep(session_exists=live, delivery_root=delivery_root, now=1000)
...
def test_session_end_reclaims_the_whole_subdirectory(store, delivery_root):
    ...
    _deliver(delivery_root, id="a", session="ended", expires_at=9999,
             now=0)
    _deliver(delivery_root, id="b", session="ended", expires_at=9999,
             now=0)
...
def test_a_release_whose_container_has_exited_is_still_destroyed(
    ...
    _deliver(delivery_root, id="r", session="dead", expires_at=1,
             now=0)
    import shutil
...
def test_a_live_session_within_deadline_is_left_untouched(store, delivery_root):
    host = _deliver(delivery_root, id="r", session="live",
                    expires_at=10_000, now=0)
    sweeper.sweep(session_exists=lambda s: True, delivery_root=delivery_root, now=1)
    assert host.exists()
...
def test_reconciliation_destroys_an_overdue_release_from_a_fresh_process(
    ...
    overdue = _deliver(delivery_root, id="old", session="s1",
                       expires_at=100, now=0)
    valid = _deliver(delivery_root, id="new", session="s2",
    ...
    rec_old = vault_releases.get("old")
    ...
    assert vault_releases.get("new")["shredded_at"] is None
...
def test_reconciliation_marks_an_overdue_live_release_reconciled_not_expired(
    ...
    _deliver(delivery_root, id="r", session="s", expires_at=100, now=0)
    sweeper.reconcile_on_startup(session_exists=lambda s: True,
                                 delivery_root=delivery_root,
                                 now=500)
    assert vault_releases.get("r")["shred_reason"] == "reconciled"
...
def test_on_session_end_marks_session_end_and_reclaims_the_subdir(
    ...
    _deliver(delivery_root, id="a", session="s", expires_at=9999, now=0)
    _deliver(delivery_root, id="b", session="s", expires_at=9999, now=0)
    out = sweeper.on_session_end("s", delivery_root=delivery_root, now=5)
    assert out == {"shredded": 2, "reclaimed_dir": True}
    assert not (delivery_root / "s").exists()
    for rid in ("a", "b"):
        assert vault_releases.get(rid)["shred_reason"] == "session_end"
