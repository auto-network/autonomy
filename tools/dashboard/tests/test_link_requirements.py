...
def _persona_record(persona_pub: str, machines: dict) -> dict:
    """A persona cut record listing *machines*, sealed by a delegate."""
    import time
    ...
    body = cuts.persona_cut_body(persona.public_hex, "g", min(machines.values()), machines, signer.public_hex)
    return {"persona": persona.public_hex, "org": "g", "cut_ns": min(machines.values()),
...
def test_requirements_fold_row_origins_to_personas_with_the_newest_stamp(tmp_path: Path) -> None:
    ...
    # M2 is attributed to persona P2 by a persona cut record the store holds.
    record = _persona_record(P2, {M2: 1_400})
    cuts.store_persona_cut(own.conn, record)
    p2 = record["persona"]
