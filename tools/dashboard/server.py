...
async def ws_voice(websocket: WebSocket):
    """Legacy voice entry point; stable gateway uses the same protocol engine."""
    from tools.dashboard.voice_transport import serve_voice
    from tools.dashboard.tmux_send import tmux_send_awaited

    await serve_voice(
        websocket, session_exists=_tmux_session_exists,
        commit_text=tmux_send_awaited,
        capture_enabled=_voice_audio_capture_enabled,
        open_capture=_open_voice_audio_capture,
        monotonic=_voice_flow_monotonic,
        incoming_max_messages=_VOICE_INCOMING_MAX_MESSAGES,
        audio_max_frame_bytes=_VOICE_AUDIO_MAX_FRAME_BYTES,
    )
...
def _load_template(name: str, **context) -> str:
    ...
    return content.replace("__STATIC_VERSION__", _static_version()).replace(
        "__VOICE_ENDPOINT_META__", voice_service_config.meta(),
    )
...
routes = [
    ...
    *([] if voice_service_config.enabled() else [WebSocketRoute("/ws/voice", ws_voice)]),
    *voice_commit_routes.ROUTES,
