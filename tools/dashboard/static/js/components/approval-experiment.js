...
export function mountApprovalExperiment(host,input,services=null){
 ...
 // Consume the shell's established iOS standalone height. In other browsers
 // the token is absent and the existing dynamic-viewport layout is unchanged.
 root.innerHTML='<style>'+approvedStyles.replace(/body\{/g,':host{').replace(/:root\{/g,':host{').replaceAll('100dvh','var(--app-height,100dvh)')+'</style>'+approvedMarkup;
 if(realDocument.documentElement.classList.contains('ios-standalone')){
   // Fixed-position iOS PWA layers can be clipped at the shorter containing
   // block. Anchor this layer in document flow using the shell's full height.
   const style=realDocument.createElement('style');
   style.textContent=':host{position:absolute!important;top:0;bottom:auto!important;height:var(--app-height,100dvh)}.auth-overlay{position:absolute}';
   root.append(style);
 }
...
$('fleet-check').hidden=model.kind!=='fleet';$('code').textContent=model.code||'';$('code-matches').checked=false;$('code-matches').disabled=false;$('machine-name').value=model.kind==='fleet'?(model.machineName||''):'';$('machine-name').disabled=false;renderTargetLink();checkFleet();renderAuth();window.ready=true;}
